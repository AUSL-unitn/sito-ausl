...
function image_resize($src, $dst, $width, $height, $crop=0){
	if(!list($w, $h) = getimagesize($src)) return "Tipo di immagine non supportata... mi disp!";
	...
	switch($type){
		...
		default : return "Tipo di immagine non supportata... mi disp!";
	}

	// resize
	if($crop){
		if($w < $width || $h < $height) return "Immagine troppo piccola!";
...
$pic_error = image_resize($pic_name, "../../IMAGES/account/allpeoples/user".$_SESSION['user_id'].".jpg", 224, 224, 1);
if ($pic_error !== true) {
	unlink($pic_name);
	...
	unlink($pic_name);
	//aggiorno il db:
	if ($mysqli->query("UPDATE users SET iHaveImage=1 WHERE id=".$_SESSION['user_id'])){
		exit('{"ok":"file caricato"}');
	}
	else exit('{"error":"impossibile aggiornare il profilo"}');
}
